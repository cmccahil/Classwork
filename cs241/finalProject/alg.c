...
struct Node *findBeats(WAVE *wave){
	...
	headBeats->next = tailBeats;  //connecting head and tail beats
	...
	for(int i = wave->sampleRate; i < wave->dataSize; i += 1024 * numChannels * bytesPerSample){//traverses songs and finds possible beats
		...
		if( (newNode->energy) > (averageEnergy * 5)  ){//node is a beat
...
int main(int argc, char *argv[]){
	...
	for(int i=0;i<wave->sampleRate;i=i+1024){ //creating the buffer 
...
//	printf("%f\n", average(head));
	...
	//printing out the beats
	struct Node *beats=findBeats(wave);
//	beats=beats->next;
