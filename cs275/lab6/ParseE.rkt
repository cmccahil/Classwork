...
(define new-let-exp
  (lambda (exp)
    (list 'let-exp (map (lambda(x) (car x)) (cadr exp)) (map (lambda(x) (cadr x)) (cadr exp)) (parse (caddr exp)))))
...
(provide parse expression new-var-ref var-ref? Symbol lit-exp? LitValue define-exp? define-exp-sym define-exp-exp new-app-exp
         apply-primitive-op prim-proc-op prim-proc? apply-proc app-exp? new-if-exp if-exp? let-exp?)